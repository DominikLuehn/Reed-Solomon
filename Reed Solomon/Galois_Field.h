...
namespace RS {
    ...
    namespace GF {
        ...
        // Addition in Form von XOR-Verknüpfung
        uint8_t add(uint8_t x, uint8_t y) {
            return x ^ y;
        }

        // macht lesen des Codes einfacher an einer Stelle
        ...
        Poly poly_add(Poly right, Poly left) {
            Poly result;
            result.Init(poly_max(right.length(), left.length()));

            for (uint8_t i = 0; i < right.length(); i++) {
                result.at(i + result.length() - right.length()) = right.at(i);
            }

            for (uint8_t i = 0; i < left.length(); i++) {
                result.at(i + result.length() - left.length()) ^= left.at(i);
            }

            return result;
        }

        Poly poly_mul(Poly right, Poly left) {
            Poly result;
            result.Init(right.length() + left.length() - 1);

            for (uint8_t j = 0; j < left.length(); j++) {
                for (uint8_t i = 0; i < right.length(); i++) {
                    result.at(i + j) ^= mul(right.at(i), left.at(j));
                }
            }
            return result;
        }

        Poly poly_div(Poly dividend, Poly divisor) {
            uint8_t coef = 0;
            for (int i = 0; i < (dividend.length() - divisor.length() - 1); i++) {
                ...
                if (coef != 0) {
                    for (int j = 1; j < divisor.length(); j++) {
                        if (divisor.at(j) != 0) {
                            dividend.at(i + j) ^= mul(divisor.at(j), coef);
                        }
                    }
                }
            }
            ...
            Poly result;
            for (int i = separator; i < dividend.length(); i++) {
                result.Append(dividend.at(i));
            }
            
            return result;
        }

        uint8_t poly_eval(Poly& poly, uint16_t x) {
            uint8_t y = poly.at(0);
            for (uint8_t i = 1; i < poly.length(); i++) {
                y = mul(y, x) ^ poly.at(i);
            }
        ...
        Poly poly_scale(Poly& source, uint16_t scalar) {
            Poly result;
            result.Init(source.length());

            for (uint16_t i = 0; i < source.length(); i++) {
                result.at(i) = mul(source.at(i), scalar);
            }
            return result;
        }
    };
